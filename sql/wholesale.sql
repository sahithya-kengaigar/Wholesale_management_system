...
CREATE TABLE `return_products` (
  `product_id` int(11) DEFAULT NULL,
  `product_name` varchar(20) DEFAULT NULL,
  `return_product_quantity` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
END
CREATE TRIGGER `return` AFTER UPDATE ON `products` FOR EACH ROW BEGIN
IF(new.quantity<=10) THEN
if (new.product_id not in (select product_id from   return_products)) THEN
INSERT INTO return_products			  VALUES(new.product_id,new.product_name,new.quantity);
ELSE
update return_products set return_product_quantity=new.quantity where product_id=new.product_id;
end if;
ELSE
delete from return_products where product_id=new.product_id;
end if;
END
$$
DELIMITER ;
...
(0, 'user', 600, 'COD', '8787', 'sdsd', '2019-03-28');
